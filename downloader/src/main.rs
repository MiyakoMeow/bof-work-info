...
async fn download_google_drive_file(file_id: &str, output_path: &Path) -> Result<PathBuf> {
...
fn extract_confirm_token_from_html(html_content: &str) -> Result<String> {
    // 在HTML中查找确认token
...
fn extract_download_url_from_html(html_content: &str) -> Option<String> {
    // 查找隐藏字段的值
    let mut id = None;
    let mut export = None;
...
fn extract_filename_from_html(html_content: &str) -> Option<String> {
...
fn extract_confirm_token(cookie_path: &Path) -> Result<String> {
    let file = fs::File::open(cookie_path)
        .with_context(|| format!("无法打开cookie文件: {:?}", cookie_path))?;

    let reader = BufReader::new(file);

    for line in reader.lines() {
        let line = line.with_context(|| "读取cookie文件失败")?;
        if line.contains("download") {
            // 提取确认token
            if let Some(start) = line.find("confirm=") {
                let token_start = start + 8;
                if let Some(end) = line[token_start..].find('&') {
                    return Ok(line[token_start..token_start + end].to_string());
                } else {
                    return Ok(line[token_start..].to_string());
                }
            }
        }
    }
...
fn is_valid_archive(file_path: &Path) -> Result<bool> {
    let file =
...
fn extract_google_drive_id_from_url(url: &str) -> Option<String> {
    // 匹配格式: https://drive.google.com/file/d/ID/view
    if let Some(start) = url.find("/file/d/") {
        let id_start = start + 8;
        if let Some(end) = url[id_start..].find("/") {
